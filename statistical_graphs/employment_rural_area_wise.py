...
def year_estimated_employed_rural():
    
    df_rural,year = fetch_dataframe()
    
    #print(year)
    
    data = df_rural.groupby(['Year'])[['Estimated Employed']].mean() #Grouped by month
...
def year_monthly_estimated_employed_rural():
    ...
    average_estimated_employed = df_rural.groupby(['Year', 'Month'])['Estimated Employed'].mean().reset_index()
    
    fig = px.bar(average_estimated_employed,x='Month',y='Estimated Employed',animation_frame='Year',
                title='Estimated Employed in rural areas from 2016 - 2021',text="Month",width=550, height=600,
...
def estimated_employed_in_rural_area_weekly_basis():
    
    df_rural,year = fetch_dataframe()
    
    fig=px.line(df_rural, x='Date', y='Estimated Employed', title='Estimated Employed in rural areas every week from 2016 to 2021',
